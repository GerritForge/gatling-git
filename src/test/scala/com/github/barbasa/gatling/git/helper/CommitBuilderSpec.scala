...
class CommitBuilderSpec extends FlatSpec with BeforeAndAfter with Matchers with GitTestHelpers {
  before {
    ...
    val response = Clone(new URIish(s"file://$originRepoDirectory"), testUser).send
  ...
  "with different destination directories" should "place files in different directories if files == directories" in {
    ...
    commitBuilder.commitToRepository(testGitRepo.getRepository)
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryA")) shouldBe 1
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryB")) shouldBe 1
  }

  "with different destination directories" should "not place files in all the directories if files < directories" in {
    val directoryA = new File(s"${testGitRepo.getRepository.getWorkTree}/directoryA")
    val directoryB = new File(s"${testGitRepo.getRepository.getWorkTree}/directoryB")
    directoryA.mkdir
    directoryB.mkdir
    ...
    commitBuilder.commitToRepository(testGitRepo.getRepository)
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryA")) shouldBe 1
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryB")) shouldBe 0
  }
  ...
  "with different destination directories" should "place files in different directories if files > directories" in {
    ...
    commitBuilder.commitToRepository(testGitRepo.getRepository)
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryA")) shouldBe 2
    getFilePathsInCommit(getHeadCommit.getTree).count(_.contains("directoryB")) shouldBe 1
  }
