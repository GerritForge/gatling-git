...
class CommitBuilder(numFiles: Int, minContentLength: Int, maxContentLength: Int, prefix: String, destinationDirectories: Seq[File]) {

  val random = new Random()

  def commitToRepository(repository: Repository) {
    val git = new Git(repository)
    val mockFileFactory = new MockFileFactory(destinationDirectories, numFiles)
