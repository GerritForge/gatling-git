...
class PushSpec extends AnyFlatSpec with BeforeAndAfter with Matchers with GitTestHelpers {
  ...
  "with multiple ref-specs" should "push all specified refs" in {
    ...
    testGitRepo.branchList.call.asScala.map(_.getName).size should be(2)
    testGitRepo.branchList.call.asScala.map(_.getName) should contain allOf (s"$R_HEADS$branch1", s"$R_HEADS$branch2")
  }
