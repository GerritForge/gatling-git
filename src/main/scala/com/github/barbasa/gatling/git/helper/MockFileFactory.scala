...
object MockFiles {
  ...
  abstract class AbstractMockFile(
    ...
    private def nextRandomName(offset: Int = 0): String = {
      val nanoTimeWithOffset = System.nanoTime() + offset
      val candidateName      = s"$prefix${nanoTimeWithOffset % totalFiles}$ext"
...
object MockFileFactory {
  ...
  def create(
    ...
    fileType match {
      case TextFileType =>
        new TextFile(contentLength, filenamePrefix, filenameExt, totalNumFiles, filenamesToAvoid)
    }
  }
}
